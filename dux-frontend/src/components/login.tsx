...
export default function LoginBox() {
  ...
  async function login() {
    ...
    try {
      const axResp = await duxServer.post(
        `auth?email=${emailInput.current?.value}&password=${passwordInput.current?.value}`
      );
      ...
      } else if (axResp.status > 300 || axResp.status < 399) setStatus(12345);
      console.log(status);
    } catch (error) {
      console.error(error);
